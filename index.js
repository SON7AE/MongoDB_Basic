...
// mongo 셸은 몽고DB를 사용하는 데 매우 중요한 도구이며 광범위하게 사용된다.

// ----------------------------------------------------------------------------------------------------

// 2-1. 셸 기본작업
// 셸에서 데이터를 조작하거나 보려면 생성(create), 읽기(read), 갱신(update), 삭제(delete)의 네 가지 기본적인 작업(CRUD)을 한다.

// 생성(create)
// inserOne 함수는 컬렉션에 도큐먼트를 추가한다.

// 읽기(read)
// find와 findOne은 컬렉션을 쿼리하는 데 사용한다.
// 컬렉션에서 단일 도큐먼트를 읽으려면 findOne을 사용한다.
// find와 findOne은 쿼리 도큐먼트 형태로 조건 전달도 가능하다. 따라서 쿼리에서 일치하는 도큐먼트로 결과를 제한한다.

// 갱신(update)
// 게시물을 갱신하려면 updateOne을 사용한다. updateOne의 매개변수는 최소 두 개다.
// 첫 번째는 수정할 도큐먼트를 찾는 기준이고, 두 번째는 갱신 작업을 설명하는 도큐먼트이다.
// 갱신하려면 갱신 연산자(update operator)인 set을 이용한다.

// 삭제(delete)
// deleteOne과 deleMany는 도큐먼트를 데이터베이스에서 영구적으로 삭제한다. 두 함수 모두 필터 도큐먼트로 삭제 조건을 지정한다.
